...
int hrzWdth = (sz-(hrstart*2) - (wdhSpace*(hrzPiece-1)))/vrtPiece ;    //ширина блока по горизонтали
int vrtHgth = (sz-(vrstart*2) - (wdhSpace*(vrtPiece-1)))/hrzPiece;    //высота блока по вертикали
...
void draw() {
  ...
  for (int i=boats.length-1, k=choise+1; i>=0; --i, --k) {
    ...
    while (!isEmpty) {
      dir = (int)random(2);
      int [] start = {
        (int)random(hrzPiece), (int)random(vrtPiece)
        };
...
void setVars () {
  ...
  //  hrzWdth = (sz-(hrstart*2) - (wdhSpace*(hrzPiece-1)))/vrtPiece;    //ширина блока по горизонтали
  //  vrtHgth = (sz-(vrstart*2) - (wdhSpace*(vrtPiece-1)))/hrzPiece ;    //высота блока по вертикали
  hrzWdth = (sz-(hrstart*2) - wdhSpace)/vrtPiece;    //ширина блока по горизонтали
  vrtHgth = (sz-(vrstart*2) - wdhSpace)/hrzPiece ;    //высота блока по вертикали

  fillBoats(boats, cells);
}

void fillBoats (int [] in, int cells) {
  int i = 0;
  for (; i<in.length; ++i) {
    in[i] = 1;
  }
  ...
  for (; i<cells; ) {
    int bufRand = (int)random(in.length);
    if (in[bufRand] != max(hrzPiece, vrtPiece)) {
      in[bufRand] += 1;
      ++i;
    }
  }
